...
### PEND-RCP-17 · G4 Single Point Calibration, escenario automático usa valores fijos, no HAL real { #pend-rcp-17 }

`src/core/control_routines/single_point_calibration_auto.py`, escenario `auto`: todas las lecturas (`noise_high_dbm`, `noise_low_dbm`, `signal_high_dbm`, `signal_low_dbm`, `hi_low_ratio_db`, `hi_channel_diff_db`, `calculated_radar_constant_db`) son constantes fijas en el código, no lecturas de `HardwareAbstractionLayer`. No existe todavía en `hal_sim/signal_catalog` un punto para comandar el generador interno de la receptora ni para leer el resultado de la medición de potencia de señal/ruido que este escenario necesita. Antes de reemplazar estos valores por lecturas reales, confirmar con el proyecto DRX qué señales Modbus exponen esa función (mismo criterio que PEND-RCP-07/PEND-RCP-16).
