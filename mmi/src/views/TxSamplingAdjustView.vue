<!--
  TxSamplingAdjustView.vue: Vista G2 — TX Sampling Adjust (RAVIS Sec.7.4.1, TX/RX adjustment).
  Permite ajustar los parámetros de muestreo TX (TX Sample, TX Frequency, Commanded LO Freq,
  TX Start/Stop Sample) con lectura cruzada en vivo contra el Scan Worksheet activo (ASCOPE D2).
-->
...
// Limite de duty cycle validado en core/contracts/scan.py (PEND-RCP-08,
// resuelto): duty = prf_hz * pulse_width_us * 1e-6, rechazado por encima de
// este valor. Se repite aqui solo como referencia visual, la guarda real
// vive en el backend del Scan Worksheet.
const DUTY_CYCLE_LIMIT = 0.00085
...
const activeCutPrfHz = ref<number | null>(null)
const activeCutPulseWidthUs = ref<number | null>(null)
const activeDutyCycle = computed(() => {
  if (activeCutPrfHz.value === null || activeCutPulseWidthUs.value === null) return null
  return activeCutPrfHz.value * activeCutPulseWidthUs.value * 1e-6
})
...
async function loadActiveScanCut() {
  try {
    const cuts = await fetchScanWorksheet()
    const first = cuts[0]
    activeCutPrfHz.value = first?.prf_hz ?? null
    activeCutPulseWidthUs.value = first?.pulse_width_us ?? null
  } catch {
    activeCutPrfHz.value = null
    activeCutPulseWidthUs.value = null
  }
}
...
<template>
  <div class="flex h-full min-h-0 flex-col gap-4 overflow-auto p-3">
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
      <!-- Panel de Lectura Cruzada ASCOPE (D2) / Scan Worksheet activo -->
      <Card class="border-border/80 bg-card/80 shadow-xs lg:col-span-1">
        <CardHeader class="border-b border-border/50 pb-2.5 pt-3.5">
          ...
          <p class="text-xs text-muted-foreground">Corte activo en Scan Worksheet</p>
        </CardHeader>
        <CardContent class="p-4 flex flex-col gap-3 text-xs">
          <div class="flex flex-col gap-1 rounded-md border border-border/50 bg-muted/20 p-2.5">
            ...
            <span class="font-mono text-sm font-semibold text-foreground">
              {{ activeCutPulseWidthUs !== null ? `${activeCutPulseWidthUs.toFixed(2)} µs` : '—' }}
            </span>
          </div>
          ...
          <div class="flex flex-col gap-1 rounded-md border border-border/50 bg-muted/20 p-2.5">
            ...
            <span class="font-mono text-sm font-semibold text-foreground">
              {{ activeCutPrfHz !== null ? `${activeCutPrfHz} Hz` : '—' }}
            </span>
          </div>

          <div class="flex flex-col gap-1 rounded-md border border-border/50 bg-muted/20 p-2.5">
            <span class="text-muted-foreground">Duty Cycle (calculado / límite)</span>
            <span class="font-mono text-sm font-semibold text-foreground">
              {{ activeDutyCycle !== null ? `${(activeDutyCycle * 100).toFixed(4)} %` : '—' }}
              / {{ (DUTY_CYCLE_LIMIT * 100).toFixed(3) }} %
            </span>
          </div>

          <p v-if="activeCutPrfHz === null" class="text-[11px] text-muted-foreground italic">
