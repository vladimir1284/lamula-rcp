...
const { status, control, antenna, dsp, fetchStatus, setControlMode, runControlJob, cancelControlJob } =
  useGateway()
...
// precondición por rutina hoy (autoridad activa + el campo obligatorio de
// cada una); el backend no expone todavía otras precondiciones físicas
// (p. ej. "sin falla BiTE de transmisor activa") que el legacy sí muestra.
const generalChecks = computed(() => [{ label: 'Autoridad de control activa', ok: isActive.value }])
const txChecks = computed(() => [
  { label: 'Autoridad de control activa', ok: isActive.value },
  { label: 'warmup_timeout_s definido', ok: txWarmupTimeoutS.value !== undefined },
])
const rxChecks = computed(() => [
  { label: 'Autoridad de control activa', ok: isActive.value },
  { label: 'confirm_timeout_s definido', ok: rxConfirmTimeoutS.value !== undefined },
])
const auChecks = computed(() => [
  { label: 'Autoridad de control activa', ok: isActive.value },
  { label: 'confirm_timeout_s definido', ok: auConfirmTimeoutS.value !== undefined },
])
  ...
  <div class="flex h-full min-h-0 flex-col gap-4 overflow-auto p-3">
    ...
    <Card>
      ...
      <CardContent class="flex flex-col gap-4">
        <PrecheckList :checks="generalChecks" />
        <JobActionPanel
